...
## Generador de informes tecnicos

- Abre `informe.html` para usar la herramienta.
- Librerias usadas (CDN): `jsPDF` y `html2canvas`.
- Credenciales de acceso: `admin / glaucusvma`.
- Lista de clientes editable en `informe.js` (constante `CLIENTES`).
- Flujo:
  - Completar datos principales.
  - Subir imagenes (foto de cargo obligatoria).
  - Click en `Generar Informe`.
  - Descarga automatica de `Informe_Cliente_Codigo.pdf`.
